...
public class LITPdhcpserviceTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPdhcpserviceTestRunner.class);
    ...
    @TestId(id = "CXP9031642-1", title = "Upgrade ERIClitpdhcpservice rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpdhcpserviceRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("dhcpservice");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031642-2", title = "Run python test cases for ERIClitpdhcpservice package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpdhcpserviceTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpdhcpserviceTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031642-3", title = "Parse xml outputs from python tests")
